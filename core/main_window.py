...
class EXREditor(QMainWindow):
    ...
    def on_tree_item_selected(self, current_item, previous_item):
        ...
        if item_type == "layer":
            ...
            r_ch = next((ch for ch in layer_channels if isinstance(ch, str) and ch.endswith(".R")),
                        None)
            g_ch = next((ch for ch in layer_channels if isinstance(ch, str) and ch.endswith(".G")),
                        None)
            b_ch = next((ch for ch in layer_channels if isinstance(ch, str) and ch.endswith(".B")),
                        None)
            a_ch = next((ch for ch in layer_channels if isinstance(ch, str) and ch.endswith(".A")),
                        None)
    ...
    def update_display(self):
        ...
        # QImage musi mieć własną kopię danych, aby nie doszło do błędu segmentacji
